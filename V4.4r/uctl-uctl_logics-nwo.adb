...
--    XSupers: Integer :=0;            --  might temporarily become negative!
  ...
  ------------------------------------------------------------------------
    --
    procedure Initialize(X: in out Fragment);
  ...
  with CPU => CPU_Range(Id),  
       STORAGE_SIZE => flags.taskstacksize;
  ...
  procedure AGForceIncompleteTrue (Id:Natural;AGIndex: Natural);
  procedure Push_Fragment (N: Fragment_Ref);
  procedure Elab_Fragment(ID:Natural; TheFrag: Fragment_Ref);
...
--        X.XSupers := X.XSupers+1;
  ...
  ---------- AG COMPUTATION DATA ------------------------
  --
  procedure Consolidate(Id:NAtural;
 ...
 function Pop_Fragment return Fragment_Ref is
  ...
  begin
    ...
    if AllEvalDone then
      ...
      if Verbose_Eval and then PoppedFrags mod 20_000 = 0 then
  ...
  procedure AGRoot_Complete (Id:Natural;AGIndex: Natural) is
  ...
  begin
    if Flags.Debug then
       Put_line("  AGRoot_Complete: " &
          AGInfo.AGCurrent.Form.Fimage.all & 
          "(" & Integer'Image(Integer(AGInfo.AGCurrent.FromState)) & ")" );
    end if;
      ...
      while AGInfo.AGQUEUE /= null loop
        ...
        if SubStatus=FOUND_TRUE or else
          ...
          while SuperFrags /= null loop
            Consolidate(Id,SuperFrags.FragRef,ItsParent,NewStatus);
  ...
  begin 
    ...
    --       TheFrag.Kind = SequentialAND
        ...
        if AGTable(TheFrag.Form.AGIndex).AGResult = NOT_YET_STARTED then
          ...
          if Flags.Debug then
             Put_line("AGRoot_Started: " &
                TheFrag.Form.Fimage.all &
                "(" & Integer'Image(Integer(TheFrag.FromState)) & ")" );
          end if;
          --
        else
          --
  ...
  ------------------------------------------------
  -- SubStatus e' FOUND_TRUE o FOUND_FALSE.
  -- Consolidate viene chiamata quando quando lo stato di SubFrag diventa Substatus.
  -- Cio' ha conseguenze per TheFrag che era nella lista dei SuperFrags di SubFrag.
  ------------------------------------------------
  procedure Consolidate(Id:Natural;
                        TheFrag: Fragment_Ref; 
                         SubFrag: Fragment_Ref;
  ...
  begin
    if TheFrag.Form = null then
      ...
      ALLResult := SubStatus;
      -- In presenza di AG  siamo sicuri che AGCOmplete non viene fatta fintanto che AGCOUNT /= 0
      -- prima di cio' tuttavia, Consolidate potrebbe aver propagato il valore FOUND_FALSE.
      -- Cio' puo' creare problemi in caso di valutazione di piu' formule.
      Set_True(ALLWait.all); 
      return;
    end if;
    if Debug then
      Put_Line("  #" & Natural'Image(Id) & " Consolidate: Sub " & FragKind'Image(SubFrag.Kind) & 
                    " (" &  Integer'Image(Progressive(TheFrag.FromState)) & ")" &
               " of "  & FragKind'Image(TheFrag.Kind) & 
                    " (" & Integer'Image(Progressive(TheFrag.FromState)) & ")" &
               " is " &  FragStatus'Image(SubStatus)); 
    end if;
    ...
    if NEWSTATUS = FOUND_TRUE or NEWSTATUS=FOUND_FALSE then
      ...
      if TheFrag.Form.Kind=Fall and then
        ...
        -- Notice: In general Consolidation can occur multiple times! the check on 
        --  SuperItems guarantees that removal is done only once.
      end if;
      --
      ...
      while SuperItems /= null loop
        ...
        Consolidate(Id,SuperItems.FragRef,TheFrag,NewStatus);
      ...
      if TheFrag.Form.Kind=Fall and then TheFrag.Form.Pref.Kind=Always and then
        ...
        --
        AGTable(TheFrag.Form.AGIndex).AGresult := FOUND_FALSE;  -- PUO NON SERVIRE!!
        --
        NEWCOUNT := Decrement32(AGTable(TheFrag.Form.AGIndex).AGCOUNT'Address);
        if NEWCOUNT=0 then
           AGForceIncompleteTrue(Id,TheFrag.Form.AGIndex);
           AGRoot_Complete(Id,TheFrag.Form.AGIndex);
        end if;
      --
      ...
      --     AGRoot_Complete(Id,TheFrag.Form.AGIndex);
          ...
          --  e AGCOunt e' appena diventato =0  
          -- Posso e Devo notificate la fine della AG
          --
          -- SAREBBE MEGLIO PER UNIFORMITA' FARE SEMPRE LA  AGRoot_Complete NEL MOMENTO
          --  IN CUI AGCOUNT DIVENTA = 0, subito dopo i Consolidamenti ed i ForceTrue.
          -- C'E' IL RISCHIO DI FORLA DUE VOLTE? E.G. anche quando AGPAR diventa TRUE?
      end if;
      --
    elsif NEWSTATUS=ACTIVATED and then TheFrag.Kind= SequentialAND then
      -- i.e.  Substatus=FOUND_TRUE e SubFrag=TheFrag.XSubFrags(1)=PHI
      if Debug then
        Put("  Consolidate: ");
      end if;
      Elab_Fragment(Id,TheFrag); 
    end if;
  end Consolidate;
  -------------------------------------------------
...
----------------------------------------------------------------------
   ------------------------------
    ...
    ------------------------------
 procedure Elab_AGSeq(Id:Natural; TheFrag: Fragment_Ref) is
    ToBePushed: Fragment_Ref;
    NextSub: Fragment_Ref;
  ...
  begin
    --
    if Flags.Debug then
       Put_Line("#" & Natural'Image(id) & " ElabAG/Seq: "&
           TheFrag.Form.Fimage.all & " (" &
...
--    end if;
    ...
    if (NextSub.Form.Kind/=Fall or else NextSub.Form.Pref.Kind/=Always or else
      ...
      if SubStatus = FOUND_FALSE then
        ...
        while SuperFrags /= null loop
          Consolidate(Id,SuperFrags.FragRef,TheFrag,NewStatus);
          SuperFrags := SuperFrags.FragNext;
        end loop;
        NextSub := null;
        --
    ...
    if NextSub /= null and then
      ...
      --
      -------------- WE ARE IN CASE B) ------------------
      -- WE ALWAYS LINK THE AGPar frag, even if the state is FINAL
      --
      ToBePushed := NextSub;  --  THIS is the PHI part of AG PHI
      ---------------------------------------------
      NextSub.Link(TheFrag,SUBSTATUS,ToBePushed);
      ...
      if SubStatus = FOUND_TRUE or SubStatus = FOUND_FALSE then
        -- CAN THIS REALLY HAPPEN?!?!?  YES IF FINAL!!
        -----------------------------------------------------------
        TheFrag.Notify(NextSub,SubStatus, SUPERFRAGS, NEWSTATUS);
        -----------------------------------------------------------
        if SubStatus = FOUND_FALSE then
           AGTable(TheFrag.Form.AGIndex).AGresult := FOUND_FALSE; -- NOT NECESSARY (probably)
        end if;
        ...
        --
        -- since we do not push AGPar, the AG completes here.
        NEWCOUNT := Decrement32(AGTable(TheFrag.Form.AGIndex).AGCOUNT'Address);
        ...
        if ToBePushed /= null then
          -- 
          ------------------------------------------
          --Increment32(AGTable(TheFrag.Form.AGIndex).AGCOUNT'Address); REMOVED
          Push_Fragment(ToBePushed);
          ------------------------------------------
    ...
    --
    if Flags.Debug then
       Put_Line("#" & Natural'Image(id) & " END ElabAG/Seq: "&
           TheFrag.Form.Fimage.all & " (" &
           Integer'Image(Progressive(TheFrag.FromState)) & ") AGCOUNT=" 
           & Counter32'Image(AGTable(TheFrag.Form.AGIndex).AGCOUNT));
    end if;
 ...
 procedure Elab_AGPar(Id:Natural;TheFrag: Fragment_Ref) is
    ToBePushed: Fragment_Ref;
    NextSub: Fragment_Ref;
    NewStatus: FragStatus := ACTIVATED;
    SubStatus: FragStatus;
  ...
  begin
    --
    if Flags.Debug then
       Put_Line("#" & Natural'Image(id) & " ElabAG/Par: "&
           TheFrag.Form.Fimage.all & " (" &
           Integer'Image(Progressive(TheFrag.FromState)) & ") AGCOUNT=" 
           & Counter32'Image(AGTable(TheFrag.Form.AGIndex).AGCOUNT));
    end if;
    ...
    while NextSub /= null loop
      ...
      if SubStatus = FOUND_TRUE or SubStatus = FOUND_FALSE then
        ...
        if (NewStatus = FOUND_TRUE or NewStatus = FOUND_FALSE) and then
          ...
          --
          exit;   -- no need the execute again TheFrag.GetNextSub
        else
    ...
    --
    if Flags.Debug then
       Put_Line("#" & Natural'Image(id) & " END ElabAG/Par: "&
           TheFrag.Form.Fimage.all & " (" &
           Integer'Image(Progressive(TheFrag.FromState)) & ") AGCOUNT=" 
           & Counter32'Image(AGTable(TheFrag.Form.AGIndex).AGCOUNT));
    end if;
...
-----------------------------------------------------------------------
  procedure Elab_Fragment(Id:Natural; TheFrag: Fragment_Ref) is
    ToBePushed: Fragment_Ref;
    NextSub: Fragment_Ref;
    NewStatus: FragStatus := ACTIVATED;
    SubStatus: FragStatus;
...
--    NEWCOUNT: Counter32;
  begin
    if TheFrag.XSubFrags=null then
        TheFrag.Initialize;
        -- can be /= JUSTCREATED when called again by a Notify
          if (TheFrag.Xstatus = FOUND_TRUE or TheFrag.Xstatus = FOUND_FALSE) and then
              TheFrag.XSuperFrags /= null then
            -- this was once done indirectly by Link by setting to null "tobepushed"
            --  which induce Notify and Consolidate
            SuperFrags := TheFrag.XSuperFrags;
            while SuperFrags /= null loop
              Consolidate(Id,SuperFrags.FragRef,TheFrag,TheFrag.Xstatus);
              SuperFrags := SuperFrags.FragNext;
              -- memo: superfrags do not change if status is FOUND_XX
              -- might work directly on  TheFrag.XSuperFrags ??
            end loop;
            return;
          end if;
    end if;
    ...
    if TheFrag.Form.Kind=Fall and then TheFrag.Form.Pref.Kind=Always then
      if TheFrag.Kind = SequentialAND then
        Elab_AGSeq(Id,TheFrag);
      else
        Elab_AGPar(Id,TheFrag);
      end if;
      return;
    end if;
    --
    if Flags.Debug then
       Put_Line("#" & Natural'Image(id) & " Elab: "&
           TheFrag.Form.Fimage.all & " (" &
           Integer'Image(Progressive(TheFrag.FromState)) & ")");
    end if;
    --
    ...
    --
    if Flags.Debug then
       Put_Line("#" & Natural'Image(id) & " Elab END: "&
           TheFrag.Form.Fimage.all & " (" &
           Integer'Image(Progressive(TheFrag.FromState)) & ")");
    end if;
  end Elab_Fragment;
...
--vieni richiamata per il nuovo ciclo co nextsu=AAG/par
  ...
  -------------------------------------------------
  procedure ForceTRUE (Id:Natural; TheFrag: Fragment_Ref) is
    SuperFrags: FragListRef;
    NewStatus: FragStatus;
  begin
    if Flags.Debug then
 ...
 ------------------------------------------------------------
 procedure AGForceIncompleteTrue(Id:Natural;AGIndex: Natural) is
   TheFrag: Fragment_Ref;
 begin
   --  From now on Consolidate will not handle AGData management
   while AGTable(AGIndex).AGIncomplete /= null loop
      TheFrag := AGTable(AGIndex).AGIncomplete.FragRef;
      if TheFrag.XStatus = ACTIVATED then
        ForceTRUE(Id,TheFrag);
        --if Flags.Debug then

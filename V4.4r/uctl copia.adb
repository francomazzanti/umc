...
--  type AssertBinOp is (NOOP, PLUS);   -- Minus, ... to be added
  ...
  True_Formula: constant Formula_Ref := new Formula(Ftrue);
  False_Formula: constant Formula_Ref := new Formula(Ffalse);
...
------------------------------------------------------------------
  ...
  subtype Computation_Step is Integer;
...
-- BUG BUG BUG HTML IS NOT OPEN/CREATED
  ...
  begin
     if Optargs = null then
      Put_Line("MESSAGE from state " & Configurations.NickName(This_Conf,"C") & ": PRINT");
      if HTML_Mode then
        Put_Line (HTML_File, "<b>MESSAGE</b> from state " & Configurations.NickName(This_Conf,"C") & ": PRINT");
      end if;
     else
      Put("MESSAGE from state " & Configurations.NickName(This_Conf,"C") & ": PRINT(");
      ...
      Put_Line(")");
      if HTML_Mode then
        Put(HTML_File, "<b>MESSAGE</b> from state " & Configurations.NickName(This_Conf,"C") & ": PRINT(");
        for I in Optargs.all'Range loop
...
--        ", evaluation time= " & Ada.Strings.Fixed.Trim (Elapsed, Left) & " sec.)<p>" );
 ...
 begin
  ...
  begin
    -----------
    -- CASE 1)  the state is FINAL, or no evoluzions satisfy the action expression,
...
--      Put_Line ("In particular:");
  ...
  --  
  --  returns Sub_Cref= the LAST subcomputation of the LAST recursive computation.
  --
  ...
  ---------------------------------------------------
  function Explain_Rec_Path(RecPath:Computations_Table;
                         Cref: Computation_Step;
  ...
  begin
    if This_Subcomputations'length > 0 then 
      ...
      Sub_Evol := This_Subevolutions(This_Subevolutions'Last);
       ...
       return Cref;   -- can happen only in the case "AG true" on a FINAL state
    end if;
    --
    if  Form /= Sub_Form  then
      ...
      if not Has_System_Transition(This_Iter) then
        ...
        if HTML_MODE then
           ...
           return Sub_Cref; ---     CAN BE UNDEFINED when Form1=True (not FINAL but no subcomptations)
        else
    ...
    for I in Previouses'Range loop
        if This_State = Previouses(I) then
          -- this is a previous state in the path
          if HTML_MODE then
            Put_Line(HTML_FILE, "&nbsp;&nbsp;&nbsp;<b>(" &
    ...
    return Explain_Rec_Path(RecPath,Sub_Cref, Previouses & This_State, HTML_File);
    --
...
--  end Explain_Rec2_Path;
  ...
  begin
    ...
    Sub_Cref := Explain_Rec_Path(RecPath,Cref, Empty_System_Configuration_Table, HTML_File);
    Sub_Form := Get_Formula(Sub_Cref);
    Sub_State := Get_State(Sub_Cref);
    --
    if HTML_Mode then
  ...
  begin
    ...
    -- in case of recursion the formula is necessarily False, so this case does not apply.
    --
...
--        Put_Line (Form.Pref.TForm.Fimage.all);
  ...
  begin
    ...
    Last_Cref := Explain_Rec_Path(RecPath,Cref, Empty_System_Configuration_Table, HTML_File);
    --
    if HTML_Mode then
      -- HTML MODE
      Put_Line(HTML_File, 
 ...
 procedure Explain_AFact_False (RecPath:Computations_Table;
  ...
  begin
    ...
    if HTML_Mode and Form.Pref.U2FormRef2 /= True_Formula then
        -- HTML MODE
        Put_Line(HTML_File,
             "<b>This happens because there is at least one full path from </b>" &
              Configurations.NickName(This_State,"C") & 
              " <b>in which all transitions either have the label which does NOT satisfy the action """ );
        Put(HTML_File,UCTL_Utilities.Aimage(Form.PRef.U2ARef2));
        Put_Line(HTML_File, """<br>");
        Put_Line(HTML_File, " or have the target which does NOT satisfy the subformula """ &
        HTML_Format(Form.Pref.U2FormRef2.Fimage.all) & """.</b>");
        --
    elsif (not HTML_Mode) and Form.Pref.U2FormRef2 /= True_Formula then
        -- TEXT MODE
        Put_Line("This happens because there is at least one full path from " &
               Configurations.NickName(This_State,"C"));
        Put_Line(" in which all transitions either have the label which does NOT satisfy the action """ &
               UCTL_Utilities.Aimage(Form.PRef.U2ARef2) & """");
        Put_Line ( " or have the target which does NOT satisfy the subformula """ &
           Form.Pref.U2FormRef2.Fimage.all & """." );
        ...
        Put_Line(HTML_File,
             "<b>This happens because there is at least one full path from </b>" &
              Configurations.NickName(This_State,"C") &
              " <b>in which all transitions DO NOT satisfy the action expression """ );
        Put(HTML_File,UCTL_Utilities.Aimage(Form.PRef.U2ARef2));
        Put_Line(HTML_File, """.<br>");
    else
        -- TEXT MODE
...
-- Put_line ("More Details not yet supported in this version...");
  ...
  procedure Explain_AUuntil1_False (RecPath:Computations_Table;
                                 Cref: Computation_Step;
                                 CurDepth:Positive;
                                 Previous: Formula_Ref;
                                 Skip_Header: Boolean := False;
  ...
  begin
    ...
    declare
      Last_Subcomputations : Computations_Table := Get_Subcomputations(Last_Cref);
      Last_Subevolutions: Evolutions_Table := Get_Subevolutions(Last_Cref);
    begin
      --
      if HTML_Mode then
        -- HTML MODE
  ...
  end Explain_AUuntil1_False;

    procedure Explain_AUuntil2_False (RecPath:Computations_Table;
                                 Cref: Computation_Step;
                                 CurDepth:Positive;
                                 Previous: Formula_Ref;
                                 Skip_Header: Boolean := False;
...
--          UCTL_Utilities.Print(Sub_Form, False);
    ...
    if Form.Kind=Fall and then
       ...
       (Form.Pref.Kind= Until1 or else Form.Pref.Kind=Wuntil1) then
      if Form.Pref.Kind= Until1 then
           Explain_AUuntil1_False(RecPath,Cref,CurDepth, Previous, Skip_Header, HTML_File);
      else
...
--      end if;
    ...
    if Form.Kind=Fall and then
       ...
       (Form.Pref.Kind= Until2 or else Form.Pref.Kind=Wuntil2) then
      if Form.Pref.Kind= Until2 then
           Explain_AUuntil2_False(RecPath,Cref,CurDepth, Previous, Skip_Header, HTML_File);
      else
